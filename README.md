...
Today Trajectory 是一个偏“时间回看”而不是单纯“时间管理”的专注工具。

很多人并不是不努力，而是在一天结束时常常会有一种模糊的流失感：明明做了不少事，却很难说清时间到底去了哪里。这个项目希望解决的正是这种感受。它用尽量轻的交互，让你在开始做一件事时留下一个专注记录，在结束时自动沉淀为一条可回看的时间轨迹。到了晚上，你看到的不再只是一个倒计时器，而是“今天我具体做了什么”。

和传统番茄钟相比，这个项目更强调三件事：

- 让开始足够轻，不被复杂设置打断
- 让记录足够真，留下任务名、开始时间、结束时间与时长
- 让回看足够直观，帮用户重新获得对一天的掌控感

目前项目已经落地为 Windows 桌面端，包含主界面、记录页、悬浮桌宠倒计时、系统托盘、结束铃声和本地持久化存储，是一个可以直接运行和继续迭代的 MVP。

## 项目亮点

- 基于 `Next.js 16 + React 19 + TypeScript + Tauri 2`
- 以桌面端为核心，支持悬浮圆形桌宠倒计时
- 支持拖动桌宠、长按结束任务、倒计时结束自动收尾
- 支持最小化到后台与系统托盘恢复
- 本地 `records.json` 明确持久化，数据位置可见、可控
- 视觉风格统一，强调纸感、留白与安静的专注氛围
